...
const AdminPartners: React.FC = () => {
  ...
  const fetchRetailers = async () => {
    ...
    const pendingRetailers = querySnapshot.docs.map(
      (doc) => ({ id: doc.id, ...doc.data() } as Retailer)
    );
  ...
  const fetchDeliveryBoys = async () => {
    ...
    const pendingDeliveryBoys = querySnapshot.docs.map(
      (doc) => ({ id: doc.id, ...doc.data() } as DeliveryBoy)
    );
        ...
        <table className="min-w-full bg-white">
          <thead>
            <tr>
              ...
              <th className="py-2 text-center">Aadhar</th>
          ...
          <tbody>
            {retailers.length > 0 &&
              retailers.map((retailer) => (
                <tr key={retailer.id}>
                  <td className="py-2 text-center">{retailer.id}</td>
                  ...
                  <td className="py-2 text-center">{retailer.identity}</td>
                  <td className="py-2 text-center">{retailer.address}</td>
                  <td className="py-2 text-center">{retailer.status}</td>
          ...
          <tbody>
            {deliveryBoys.length > 0 &&
              deliveryBoys.map((dBoy) => (
                <tr key={dBoy.id}>
                  <td className="py-2 text-center">{dBoy.id}</td>
                  <td className="py-2 text-center">{dBoy.name}</td>
                  <td className="py-2 text-center">{dBoy.email}</td>
                  <td className="py-2 text-center">{dBoy.phoneNumber}</td>
                  <td className="py-2 text-center">{dBoy.identity}</td>
                  <td className="py-2 text-center">{dBoy.address}</td>
                  <td className="py-2 text-center">{dBoy.status}</td>
                  <td className="py-2 text-center">
                    <button
        ...
        {showModalDB && (
          <div className="fixed inset-0 flex items-center justify-center bg-gray-500 bg-opacity-75">
            <div className="bg-white p-4 rounded-lg shadow-lg w-1/3">
              ...
              <div className="flex justify-end">
                <button
                  onClick={closeModalDB}
                  className="p-2 bg-gray-500 text-white rounded mr-2"
                >
                  Close
                </button>
                <button
                  onClick={handleRejectionDB}
                  className="p-2 bg-red-500 text-white rounded"
                >
                  Submit
                </button>
